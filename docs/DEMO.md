...
| 5 | Trade Desk | Proposed SELL NVDA → Preview → Execute → fill, mark, P&L since fill; performance chart | Advisement becomes an order on the demo paper connection (simulated fills; a real IBKR paper session drops in when logged in); the fill feeds the next run |
...
Fallbacks: model busy → deterministic report (badge shows it). Broker: with no IBKR login the desk runs as a **demo connection** (status pill "IBKR demo") and fills instantly at the last known mark; nothing reaches a broker.
